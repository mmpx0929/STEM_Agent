﻿<template>
  <view class="page">
    ...
    <view class="category-card" v-for="category in catalog.categories" :key="category.key">
      ...
      <view class="item-list">
        <view
          ...
          :key="item.id"
          :class="{ 'is-locked': !isVideoUnlocked || isExperimentComingSoon(item), 'is-coming-soon': isExperimentComingSoon(item) }"
        ...
        >
          ...
          <view class="item-action">
            <text class="action-icon">{{ experimentActionIcon(item) }}</text>
            <text>{{ experimentActionText(item) }}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <AIChat
      ...
      pageContext="home"
      guide-key="index"
      guide-audio="/static/audio/home.mp3"
      :auto-play="true"
    />
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    isExperimentComingSoon(item) {
      return item && item.status === 'coming-soon';
    },
    experimentActionIcon(item) {
      if (this.isExperimentComingSoon(item)) {
        return '⏳';
      }
      return this.isVideoUnlocked ? '🚀' : '🔒';
    },
    experimentActionText(item) {
      if (this.isExperimentComingSoon(item)) {
        return '敬请期待';
      }
      return this.isVideoUnlocked ? '开始实验' : '观看视频后解锁';
    },
    openExperiment(item) {
      if (this.isExperimentComingSoon(item)) {
        uni.showToast({
          title: '该实验内容正在准备中，敬请期待...',
          icon: 'none',
          duration: 1800
        });
        return;
...
.item-card.is-coming-soon {
  border-color: rgba(139, 154, 179, 0.45);
  background: linear-gradient(135deg, #f6f8fb 0%, #fff9ef 100%);
}

.item-card.is-coming-soon .item-action {
  background: linear-gradient(135deg, #8b9ab3 0%, #b6a070 100%);
  box-shadow: none;
}
