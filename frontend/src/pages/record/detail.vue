              ...
              <view class="data-row sub">
                ...
                <view class="data-col-condition no-padding">
                  ...
                  >
                    ...
                    >
                      <image class="data-evidence-image" :src="img.path" mode="aspectFill" lazy-load decoding="async" />
...
export default {
  ...
  computed: {
    ...
    aiGuideAudio() {
      if (this.type === 'plan') return '/static/audio/record_1.mp3';
      if (this.type === 'data') return '/static/audio/record_2.mp3';
      return '/static/audio/record_3.mp3';
    },
    detailSections() {
  ...
  methods: {
    ...
    printCurrentRecord() {
      ...
      const reservedPrintWindow = window.open('', '_blank');
      if (!reservedPrintWindow) {
        uni.showToast({
          title: '浏览器拦截了打印页，请允许弹窗后重试',
          icon: 'none'
        });
        return;
      }
      ...
      this.$nextTick(() => {
        setTimeout(() => {
          const opened = this.openPrintableRecordWindow(reservedPrintWindow);
    ...
    openPrintableRecordWindow(printWindow) {
      ...
      pageClone.querySelectorAll('.top-row, .ai-chat-print-hidden, .section-toggle').forEach((node) => node.remove());
      pageClone.querySelectorAll('.plan-form-wrap').forEach((node) => {
        node.style.display = 'none';
      });
      pageClone.querySelectorAll('.plan-print-wrap').forEach((node) => {
      ...
      const escapeHtml = (value = '') => String(value)
        ...
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;');
      const title = escapeHtml(`${this.record && this.record.experimentName ? this.record.experimentName : '实验记录'}-${this.typeLabel}`);
      const printCss = `
        <style>
          html, body {
            margin: 0;
