...
<template>
  <view class="ai-chat-container">
    <view
      ...
      @touchend.stop.prevent="playGuide"
      @click.stop="playGuide"
    >
      <text class="guide-text">{{ activeGuideText || '点我就能听语音引导哦' }}</text>
      <text class="guide-icon">{{ guideBubbleIcon }}</text>
      <view class="guide-close" @touchend.stop.prevent="closeGuideBubble" @click.stop="closeGuideBubble">×</view>
    </view>
          ...
          <view class="message-item" :class="msg.role">
            <view class="message-content" v-if="msg.role === 'assistant'">
              ...
              <view class="bubble">
                ...
                <view class="source-panel" v-if="showBackendDebug && msg.sources && msg.sources.length">
                ...
                <view class="debug-panel" v-if="showBackendDebug && msg.retrieval">
                  <text class="debug-title">后端过程</text>
                  <text class="debug-line" v-for="(line, debugIndex) in formatRetrievalDebug(msg.retrieval)" :key="debugIndex">{{ line }}</text>
                </view>
                <view class="bubble-footer">
                  <text class="bubble-time">{{ formatTime(msg.timestamp) }}</text>
                  <text class="speak-btn" @touchend.stop.prevent="speakText(msg.content)" @click.stop="speakText(msg.content)">&#128266;</text>
                </view>
              </view>
            </view>
...
const AI_MEDIA_UNLOCK_STORAGE_KEY = 'stem_ai_media_unlocked';
...
export default {
  ...
  computed: {
    ...
    chatWindowStyle() {
      if (this.windowWidth <= 520) {
        const mobileWidth = Math.max(300, this.windowWidth - 16);
        const mobileHeight = Math.max(420, this.windowHeight - 24);
        return {
          position: 'fixed',
          left: '8px',
          top: '8px',
    ...
    messageListHeight() {
      if (this.windowWidth <= 520) {
        return Math.max(240, this.windowHeight - 250);
      }
    ...
    guideBubbleStyle() {
      ...
      const style = {
        position: 'fixed',
        zIndex: 10020,
        maxWidth: this.windowWidth <= 520 ? `${Math.max(240, this.windowWidth - 112)}px` : '360px',
        padding: '12px 16px',
        borderRadius: '16px',
        background: 'white',
  ...
  mounted() {
    ...
    this.hasSpeechUserGesture = this.readMediaUnlockFlag();
    ...
    // 自动播放语音引导（等待语音列表准备完成）
    this.$nextTick(() => {
      this.startInitialGuidePlayback();
    });
  },
  beforeDestroy() {
    this.stopGuide();
  ...
  methods: {
    ...
    formatRetrievalDebug(retrieval = {}) {
      const lines = [];
      const normalized = retrieval.normalized_context || {};
      const route = retrieval.route_decision || {};
      const rewritten = retrieval.rewritten_query || {};
      const query = retrieval.query || {};
      const cache = retrieval.cache || {};
      const filters = rewritten.filters || {};
      lines.push(`路由：${retrieval.route || route.route || 'unknown'} / use_kb=${route.use_kb === undefined ? 'unknown' : route.use_kb}`);
      lines.push(`实验：${normalized.resolved_experiment_id || query.experiment_id || filters.experiment_id || '未命中'}`);
      lines.push(`文档：${query.doc_type || filters.doc_type || normalized.doc_type || '未限定'}，步骤：${query.step_id || filters.step_id || normalized.resolved_step_id || '未限定'}`);
      if (rewritten.retrieval_query) {
        lines.push(`改写：${String(rewritten.retrieval_query).slice(0, 90)}`);
      }
      lines.push(`候选：vector=${retrieval.vector_candidates ?? 0}，bm25=${retrieval.sparse_candidates ?? 0}，融合=${retrieval.fused_candidates ?? 0}，重排=${retrieval.reranked_candidates ?? retrieval.matched ?? 0}`);
      lines.push(`缓存：retriever=${cache.retriever_cache_hit === true}，query=${cache.query_cache_hit === true}`);
      if (Array.isArray(normalized.warnings) && normalized.warnings.length) {
        lines.push(`提示：${normalized.warnings.join('；')}`);
      }
      return lines;
    },
    ...
    startInitialGuidePlayback() {
      if (!this.autoPlay) return;
      if (!this.activeGuideText && !this.activeGuideAudio) return;
      this.showGuideBubble = true;
      this.waitForVoicesAndPlayGuide();
      this.clearInitialGuideRetries();

      // 首次进入页面时，移动端/部分桌面浏览器的语音列表可能晚于 mounted 才可用。
      // 这里补两次轻量重试，避免只有切换页面后才触发播报。
      [2200, 4200].forEach((delay) => {
        const timer = setTimeout(() => {
          this.initialGuideRetryTimers = this.initialGuideRetryTimers.filter((item) => item !== timer);
          if (!this.autoPlay) return;
          if (this.isGuidePlaying) return;
          if (!this.activeGuideText && !this.activeGuideAudio) return;
          this.tryAutoPlayGuide();
        }, delay);
        this.initialGuideRetryTimers.push(timer);
      });
    },
    clearInitialGuideRetries() {
      if (!Array.isArray(this.initialGuideRetryTimers)) {
        this.initialGuideRetryTimers = [];
        return;
      }
      this.initialGuideRetryTimers.forEach((timer) => clearTimeout(timer));
      this.initialGuideRetryTimers = [];
    },
    getGuideRecordKey() {
    ...
    initSpeechVoices() {
      ...
      const loadVoices = () => {
        try {
          ...
          if (voices.length > 0) {
            ...
            if (this.autoPlay && this.showGuideBubble && !this.isGuidePlaying) {
              this.tryAutoPlayGuide();
            }
          }
        } catch (error) {
      ...
      if (!this.onVoicesChangedHandler && typeof speechSynthesis.addEventListener === 'function') {
        this.onVoicesChangedHandler = loadVoices;
        speechSynthesis.addEventListener('voiceschanged', this.onVoicesChangedHandler);
      }
    },
    // 等待语音列表后再触发自动播报
    waitForVoicesAndPlayGuide() {
      ...
      // 语音列表已就绪，直接尝试播放
      if (this.voicesLoaded || (speechSynthesis && speechSynthesis.getVoices && speechSynthesis.getVoices().length > 0)) {
        this.tryAutoPlayGuide();
        return;
      }
      ...
      this.voicePollingTimer = setInterval(() => {
        ...
        const runtimeSpeech = this.getSpeechSynthesis();
        if (this.voicesLoaded || (runtimeSpeech && runtimeSpeech.getVoices && runtimeSpeech.getVoices().length > 0)) {
          clearInterval(this.voicePollingTimer);
          this.voicePollingTimer = null;
          this.tryAutoPlayGuide();
    ...
    onDrag(event) {
      ...
      // 手机轻触会有细小抖动，阈值过低会把点击误判成拖拽。
      const dragThreshold = event && event.type && event.type.indexOf('touch') === 0 ? 10 : 4;
    ...
    cleanupListeners() {
      ...
      if (
        ...
        speechSynthesis.removeEventListener('voiceschanged', this.onVoicesChangedHandler);
      }
    ...
    getSpeechUtteranceCtor() {
      if (typeof window === 'undefined') return null;
      return window.SpeechSynthesisUtterance || window.webkitSpeechSynthesisUtterance || null;
    },
    canUseTts() {
      return !!(this.getSpeechSynthesis() && this.getSpeechUtteranceCtor());
    },
    readMediaUnlockFlag() {
      try {
        return uni.getStorageSync(AI_MEDIA_UNLOCK_STORAGE_KEY) === true;
      } catch (e) {
        return false;
      }
    },
    markMediaUnlocked() {
      ...
      try {
        uni.setStorageSync(AI_MEDIA_UNLOCK_STORAGE_KEY, true);
      } catch (e) {
    ...
    playGuide() {
      ...
      if (
        ...
        window.__STEM_ACTIVE_GUIDE_OWNER__ &&
        window.__STEM_ACTIVE_GUIDE_OWNER__ !== this &&
        typeof window.__STEM_STOP_ACTIVE_GUIDE__ === 'function'
      ) {
        window.__STEM_STOP_ACTIVE_GUIDE__();
      }
      if (typeof window !== 'undefined') {
        window.__STEM_ACTIVE_GUIDE_OWNER__ = this;
        window.__STEM_STOP_ACTIVE_GUIDE__ = () => {
          if (this && typeof this.stopGuide === 'function') {
            this.stopGuide();
          }
        };
      }
      ...
      if (this.activeGuideAudio) {
        ...
        this.innerAudioContext.onError((res) => {
          ...
          this.guidePlaybackMode = '';
          this.unlockMediaPlayback();
          if (this.activeGuideText && this.canUseTts()) {
      ...
      } else if (this.activeGuideText && this.canUseTts()) {
        this.guidePlaybackMode = 'tts';
        this.speakText(this.activeGuideText, () => {
          this.isGuidePlaying = false;
          this.isGuidePaused = false;
    ...
    pauseGuide() {
      ...
      try {
        if (this.guidePlaybackMode === 'audio' && this.innerAudioContext) {
          ...
          this.guidePlaybackMode === 'tts' &&
          this.getSpeechSynthesis() &&
          this.getSpeechSynthesis().speaking
        ) {
          this.getSpeechSynthesis().pause();
        }
      } catch (e) {
    ...
    resumeGuide() {
      ...
      try {
        if (this.guidePlaybackMode === 'audio' && this.innerAudioContext) {
          this.innerAudioContext.play();
        } else if (this.guidePlaybackMode === 'tts') {
          const speechSynthesis = this.getSpeechSynthesis();
    ...
    stopGuide() {
      ...
      if (typeof window !== 'undefined' && window.__STEM_ACTIVE_GUIDE_OWNER__ === this) {
        window.__STEM_ACTIVE_GUIDE_OWNER__ = null;
        window.__STEM_STOP_ACTIVE_GUIDE__ = null;
      }
    },
    closeGuideBubble() {
      this.stopGuide();
    ...
    speakText(text, onEnd) {
      ...
      const speechSynthesis = this.getSpeechSynthesis();
      const SpeechUtterance = this.getSpeechUtteranceCtor();
      if (!speechSynthesis || !SpeechUtterance) {
        this.showRuntimeStatus('当前浏览器没有可用的系统语音，先看文字引导继续操作。', 'info', 4200);
        if (onEnd) onEnd();
        return;
      }
      ...
      try {
        ...
        const buildUtterance = (voiceMode = 'preferred') => {
          const utterance = new SpeechUtterance(content);
          const voiceList = speechSynthesis.getVoices ? (speechSynthesis.getVoices() || []) : [];
        ...
        const runSpeak = (voiceMode = 'preferred', isRetry = false) => {
          ...
          utterance.onerror = (event) => {
            ...
            this.showRuntimeStatus('当前浏览器没有启动系统语音，请先看文字引导继续操作。', 'info', 3200);
            finishOnce();
          };
          ...
          this.speechStartProbeTimer = setTimeout(() => {
            this.speechStartProbeTimer = null;
            if (!isTaskActive()) return;
            if (finished || started || speechSynthesis.speaking) return;
            if (!isRetry) {
              runSpeak('zh-fallback', true);
              return;
            }
            this.showRuntimeStatus('当前浏览器没有启动系统语音，请先看文字引导继续操作。', 'info', 3600);
            finishOnce();
          }, isRetry ? 700 : 450);
        };
