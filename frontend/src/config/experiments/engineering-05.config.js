...
const introVideo = (name) => INTRO_VIDEO_BASE + '/' + name + '.mobile.mp4';
const eng1Image = (name) => '/static/experiments/engineering-01/images/' + toMobileImagePath(name);
const eng2Image = (name) => '/static/experiments/engineering-02/images/' + toMobileImagePath(name);
const eng5Image = (name) => '/static/experiments/engineering-05/images/' + toMobileImagePath(name);
...
const flowTemplate = withOverride(BASE_TEMPLATE, {
    ...
    step4: {
	  ...
	  blueprints: [
	    { name: '模型图纸设计 1', path: eng5Image('4-1.png') },
	    { name: '模型图纸设计 2', path: eng5Image('4-2.png') }
	  ],
	  buildStepOptions: [
	    {
	      key: 'step1',
	    ...
	    {
	      key: 'step4',
	      title: '模型搭建步骤：组装电动洗衣机',
	      summary: '组装电动洗衣机。',
	      lines: ['使用系统：电动动力系统、外壳系统，使用材料：皮带轮泡棉胶、透明胶、波轮/涡轮，使用工具：剪刀（自备）']
	    }
	  ],
      buildSteps: [
        '步骤1：准备搭建材料和工具。',
    ...
    step7: {
	  ...
	  testGuideVideo: {
	    ...
	    videos: [
	      { title: '测试流程 1', videoUrl: '/static/experiments/engineering-05/videos/8-1.mobile.mp4' },
	      { title: '测试流程 2', videoUrl: '/static/experiments/engineering-05/videos/8-2.mobile.mp4' }
	    ]
	  },
      records: [
        {
        ...
        },
        {
		  ...
		  effect: '',
		  effectByChoice: {
		    A: '部分达标：但是袜子没有办法和水充分碰撞摩擦，不能把袜子洗得很干净。',
		    B: '不达标：袜子拧成一团，没有办法和水充分碰撞摩擦，不能把袜子洗得很干净。'
		  }
        },
        {
          key: 'operation',
		  ...
		  effect: '',
		  effectByChoice: {
		    A: '部分达标：操作流程基本清晰，但仍需要继续优化内部搅动方式。',
		    B: '不达标：操作流程比较混乱，需要继续优化内部搅动方式。'
		  }
        },
        {
          key: 'safety',
...
const config = {
  ...
  status: 'enabled',
  intro: {
    homeVideoUrl: '/static/Introductory-video/home.mobile.mp4',
    step1VideoUrl: '/static/experiments/engineering-05/videos/intro.mobile.mp4'
  },
  voiceGuide: {
    stepGuideTextMap: ENGINEERING_STEP_GUIDE_TEXT_MAP,
    stepSceneMap: ENGINEERING_STEP_SCENE_MAP
