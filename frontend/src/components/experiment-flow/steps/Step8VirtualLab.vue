﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿<template>
  <view class="step-card">
        ...
        <view class="zone workspace">
          ...
          <view class="workspace-canvas" :class="{ hot: isWorkspaceHot }">
            ...
            >
              <image
                class="part-image"
                :src="resolveImageSrc(resolveItem(part.itemId).image)"
                mode="aspectFit"
                lazy-load
                decoding="async"
                @load="onImageLoad(part.itemId, $event)"
                @error="onImageError(resolveItem(part.itemId).image)"
              />
              ...
              >查看</view>
              <view
                v-if="!part.isCraftPreview"
                class="part-close"
                @touchstart.stop.prevent
                @touchend.stop.prevent="removeWorkspacePart(index)"
                @mousedown.stop.prevent
                @click.stop="removeWorkspacePart(index)"
              >×</view>
            </view>
          </view>
      </view>
      ...
      <view class="op-layout" :class="{ 'has-practice': practiceVideo && practiceVideo.videoUrl }">
          ...
          <view class="op-player-body">
            <view v-if="currentOperation && currentOperation.videoUrl" class="video-frame">
              <video
                ...
                :src="resolveMediaSrc(currentOperation.videoUrl)"
                :autoplay="false"
                controls
        ...
        <view class="video-panel" v-if="practiceVideo && practiceVideo.videoUrl">
          ...
          <view class="video-frame">
            <video
              class="video-frame-player"
              :src="resolveMediaSrc(practiceVideo.videoUrl)"
              controls
              :show-center-play-btn="true"
              :enable-progress-gesture="true"
              playsinline
    ...
    <view v-if="poolDrag.active" class="drag-ghost" :style="{ left: poolDrag.x + 'px', top: poolDrag.y + 'px' }">
      <image v-if="poolDrag.itemImage" class="drag-ghost-image" :src="resolveImageSrc(poolDrag.itemImage)" mode="aspectFit" decoding="async" @error="onImageError(poolDrag.itemImage)" />
  ...
  methods: {
    ...
    dragPart(index, event) {
      ...
      const distance = Math.sqrt(
        Math.pow(point.x - this.draggingPart.startX, 2) + Math.pow(point.y - this.draggingPart.startY, 2)
      );
...
.part-close { position: absolute; right: -18rpx; top: -18rpx; width: 44rpx; height: 44rpx; border-radius: 999rpx; background: #f35a5a; color: #fff; border: 3rpx solid #fff; display: flex; align-items: center; justify-content: center; font-size: 28rpx; line-height: 1; z-index: 8; box-shadow: 0 4rpx 10rpx rgba(189, 51, 51, 0.28); }
.results {
  min-height: 0;
  display: flex;
  flex-direction: column;
