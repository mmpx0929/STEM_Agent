  ...
  <view class="step-card">
    ...
    <view class="lab-grid">
      <view class="zone pool-zone">
        ...
        <view class="token-grid">
          ...
          >
            <image class="token-image" :src="resolveImageSrc(item.image)" mode="aspectFit" lazy-load decoding="async" @error="onImageError(item.image)" />
      ...
      <view class="right-column">
        <view class="zone workspace-zone">
          ...
          <view class="workspace" :class="{ hot: isWorkspaceHot }">
            ...
            >
              <image class="part-image" :src="resolveImageSrc(itemMap[part.itemId].image)" mode="aspectFit" lazy-load decoding="async" @error="onImageError(itemMap[part.itemId].image)" />
              ...
              <view
                class="part-del"
                @touchstart.stop.prevent
                @touchend.stop.prevent="removePart(index)"
                @mousedown.stop.prevent
        ...
        <view class="zone results-zone">
          ...
          <view class="result-list">
            ...
            >
              <image class="result-image" :src="resolveImageSrc(item.image)" mode="aspectFit" lazy-load decoding="async" @error="onImageError(item.image)" />
              <view
      ...
      <view class="video-grid">
        <view class="video-card" v-for="item in supportVideos" :key="item.id">
          ...
          <view class="video-frame" v-if="item.videoUrl">
            <video class="video-player" :src="resolveMediaSrc(item.videoUrl)" controls show-center-play-btn playsinline webkit-playsinline preload="metadata" object-fit="cover" />
          </view>
    ...
    <view v-if="preview.visible" class="preview-mask" @click="closePreview">
      <view class="preview-card" @click.stop>
        ...
        <image v-if="preview.src" class="preview-image" :src="resolveImageSrc(preview.src)" mode="aspectFit" decoding="async" @error="onImageError(preview.src)" />
      </view>
    </view>
  </view>
</template>
...
import { encodeStaticAssetPath } from '@/config/media-utils.js';
...
export default {
  ...
  data() {
    return {
      ...
      draggingPart: { active: false, index: -1, startX: 0, startY: 0, startLeft: 0, startTop: 0, moved: false },
  ...
  methods: {
    resolveMediaSrc(src = '') {
      return encodeStaticAssetPath(src);
    },
    resolveImageSrc(src = '') {
      const raw = String(src || '').trim();
      if (!raw) return '';
      if (this.imageFallbackMap[raw]) return this.resolveMediaSrc(raw);
      if (/\.mobile\.jpg($|\?)/i.test(raw)) return this.resolveMediaSrc(raw);
      if (!/\.(png|jpe?g)($|\?)/i.test(raw)) return this.resolveMediaSrc(raw);
      return this.resolveMediaSrc(raw.replace(/\.(png|jpe?g)($|\?)/i, '.mobile.jpg$2'));
    },
    onImageError(src = '') {
      const raw = String(src || '').trim();
      if (!raw || this.imageFallbackMap[raw]) return;
      this.imageFallbackMap = {
        ...this.imageFallbackMap,
        [raw]: true
      };
    },
    ...
    startPartDrag(index, event) {
      ...
      if (!part) return;
      const p = this.getPoint(event);
      this.draggingPart = { active: true, index, startX: p.x, startY: p.y, startLeft: part.left, startTop: part.top, moved: false };
    },
    dragPart(index, event) {
      if (!this.draggingPart.active || this.draggingPart.index !== index) return;
      const p = this.getPoint(event);
      const distance = Math.sqrt(
        Math.pow(p.x - this.draggingPart.startX, 2) + Math.pow(p.y - this.draggingPart.startY, 2)
      );
      if (!this.draggingPart.moved && distance < 10) return;
      if (!this.draggingPart.moved) {
        this.draggingPart = { ...this.draggingPart, moved: true };
      }
    ...
    endPartDrag(index) {
      if (!this.draggingPart.active || this.draggingPart.index !== index) return;
      this.draggingPart = { active: false, index: -1, startX: 0, startY: 0, startLeft: 0, startTop: 0, moved: false };
...
.part-del { position: absolute; right: -12px; top: -12px; width: 28px; height: 28px; border-radius: 50%; background: #f35a5a; color: #fff; text-align: center; line-height: 28px; font-size: 20px; z-index: 8; border: 2px solid #fff; box-shadow: 0 2px 7px rgba(189, 51, 51, 0.28); }
.results-zone {
  min-height: 0;
  display: flex;
  flex-direction: column;
