<template>
  <view class="step-card">
    ...
    <view class="section">
      ...
      <view class="video-card" v-if="testGuideVideos.length">
        ...
        <view class="video-list" :class="{ 'single-video': testGuideVideos.length === 1 }">
          <view class="video-item" v-for="(item, index) in testGuideVideos" :key="item.videoUrl || item.url || index">
            <view class="video-item-title" v-if="item.title">{{ item.title }}</view>
            <view class="video-frame">
              <video
                class="guide-video"
                :src="resolveGuideVideoSrc(item)"
                :poster="resolveGuidePosterSrc(item)"
                controls
...
export default {
  ...
  computed: {
    ...
    testGuideVideos() {
      const config = this.testGuideVideo || {};
      if (Array.isArray(config.videos)) {
        return config.videos
          .map((item) => (typeof item === 'string' ? { videoUrl: item } : item))
          .filter((item) => item && (item.videoUrl || item.url));
      }
      return config.videoUrl ? [{ title: config.videoTitle || '', videoUrl: config.videoUrl }] : [];
    },
    records() {
  ...
  methods: {
    resolveGuideVideoSrc(item = {}) {
      return encodeStaticAssetPath(item.videoUrl || item.url || '');
    },
    resolveGuidePosterSrc(item = {}) {
      const explicitPoster = item.poster || item.posterUrl;
      if (explicitPoster) return encodeStaticAssetPath(explicitPoster);
      const videoUrl = String(item.videoUrl || item.url || '').trim();
      const derivedPoster = videoUrl.replace(/\.mobile\.mp4($|\?)/i, '.poster.mobile.jpg$1');
      return derivedPoster === videoUrl ? '' : encodeStaticAssetPath(derivedPoster);
    },
    patch(payload) {
...
.video-list.single-video .video-item {
  grid-column: 1 / -1;
  justify-self: center;
